...
console.log('data',data)
  // if(!data.link || !data.format){
  //   return 
  // }
       var urlType = data.format.replace(/\s*/g,"");;
       var link = data.link;
       if(urlType == 'gallery'){
        console.log('进入图片详情页')
     var id = data.id,
...
// return
  ...
  fetchDetailData: function (id) {
    ...
    getPostDetailRequest
      .then(response => {
        res = response;
            //  进入详情页判断内容类型，然后调整对应页面
    var isreturn =  self.swatchToLink(response.data) 
     if(isreturn){
       return
     }
